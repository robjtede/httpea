pub(crate) static STATIC_TABLE: [(&[u8], &[u8]); 99] = [
    (b":authority", b""),
    (b":path", b"/"),
    (b"age", b"0"),
...
mod tests {
    ...
    #[test]
    fn indices() {
        assert_eq!(&STATIC_TABLE[25].0, b":status");
        assert_eq!(&STATIC_TABLE[25].1, b"200");

        assert_eq!(&STATIC_TABLE[74].0, b"access-control-allow-credentials");
        assert_eq!(&STATIC_TABLE[74].1, b"TRUE");
    }
}
